...
namespace Rosier.Blog.Services
{
    ...
    {
        protected IWebOperationContext webOperationContext;
        ...
        /// The URI template to get the Atom feed representing the collection. The URL is of the form http://<url-for-svc-file>/collection1
        /// </summary>
        public const string RssFeedTemplate = "{collection}/Feed/RSS";
        /// <summary>
        /// The URI template to get the Atom feed representing the collection. The URL is of the form http://<url-for-svc-file>/collection1
        /// </summary>
        public const string AtomFeedTemplate = "{collection}/Feed/Atom";
        /// <summary>
        ...
        /// <param name="hasMoreEntries">if set to <c>true</c> [has more entries].</param>
        /// <returns></returns>
        ...
        protected Uri GetEntryUri(string collection, string id)
        {
            return entryTemplate.BindByPosition(this.webOperationContext.BaseUri, collection, id);
        }

        protected Uri GetMediaItemUri(string collection, string id)
        {
            return mediaItemTemplate.BindByPosition(this.webOperationContext.BaseUri, collection, id);
        }

        protected Uri GetAllEntriesUri(string collection)
        {
            return allEntriesTemplate.BindByPosition(this.webOperationContext.BaseUri, collection);
        }

        protected void SetAtomServiceDocumentContentType()
        {
            ...
            this.webOperationContext.OutgoingContentType = "text/xml";
        }

        private void SetAtomContentType()
        {
            this.webOperationContext.OutgoingContentType = ContentTypes.Atom;
        }

        private void SetAtomEntryContentType()
        {
            this.webOperationContext.OutgoingContentType = ContentTypes.AtomEntry;
        }

        private string GetDescriptionFromSlugHeader()
        {
            return this.webOperationContext.GetDescriptionFromSlughHeader();
        }

        protected SyndicationLink CreateEditLink(string collection, string id)
        {
            return new SyndicationLink(GetEntryUri(collection, id), "edit", "Edit Atom entry", ContentTypes.AtomEntry, 0);
        }

        protected SyndicationLink CreateEditMediaLink(string collection, string id, string contentType)
        {
        ...
        protected SyndicationLink GetNextPageLink(string collection, int pageNumber)
        {
            Uri feedUri = pagedEntriesTemplate.BindByPosition(this.webOperationContext.BaseUri, collection, pageNumber.ToString(CultureInfo.InvariantCulture));
        ...
        protected void ConfigureAtomEntry(string collection, SyndicationItem entry, string id, out Uri location)
        {
        ...
        public AtomPub10ServiceDocumentFormatter GetDocument()
        {
            ...
            {
                this.serviceDocument = GetServiceDocument();
            }
            SetAtomServiceDocumentContentType();
            return (AtomPub10ServiceDocumentFormatter)this.serviceDocument.GetFormatter();
        }

        public Rss20FeedFormatter GetRss20Feed(string collection)
        {
        ...
        public Atom10FeedFormatter GetAllEntriesAsAtom(string collection)
        {
            return GetAtomFeed(collection);
        }

        public Atom10FeedFormatter GetAtomFeed(string collection)
        {
            if (!IsValidCollection(collection))
            {
                WebOperationContext.Current.OutgoingResponse.SetStatusAsNotFound();
            ...
            int startIndex = pageNumber*this.GetMaximumEntriesInFeed(collection);
            bool hasMoreEntries;
            IEnumerable<SyndicationItem> entries = GetEntries(collection, startIndex,
                                                              this.GetMaximumEntriesInFeed(collection),
                                                              out hasMoreEntries);
            if (entries == null) entries = new List<SyndicationItem>();
            SyndicationFeed feed = CreateFeed(collection);
            feed.Items = entries;
        ...
        public Atom10ItemFormatter AddEntry(string collection, Stream stream)
        {
            ...
            string contentType = this.webOperationContext.IncomingContentType;
            ...
            this.webOperationContext.SetStatusAsCreated(location);
        ...
        /// <summary>
        /// Determines whether this request is authenticated.
        /// </summary>
        /// <returns>
        ///   <c>true</c> if this request is authenticated; otherwise, <c>false</c>.
        /// </returns>
        /// <remarks>
        /// If not authenticated, it will set the status code to Unauthorized.
        /// </remarks>
        protected bool IsAuthenticated()
        {
            var context = WebOperationContext.Current;

            string authorizationHeader = context.IncomingRequest.Headers[HttpRequestHeader.Authorization];
            if (authorizationHeader == null)
            {
                context.OutgoingResponse.Headers.Add("WWW-Authenticate: Basic realm=\"ronaldrosierdotnet\"");
                //this.webOperationContext.OutgoingStatusCode = HttpStatusCode.Unauthorized;
                return false;
            }
            return true;
        }
    ...
    public interface IAtomPubService
    {
        ...
        [WebGet(UriTemplate = AtomPubServiceBase.RssFeedTemplate)]
        [OperationContract]
        Rss20FeedFormatter GetRss20Feed(string collection);

        [WebGet(UriTemplate = AtomPubServiceBase.AtomFeedTemplate)]
        [OperationContract]
        Atom10FeedFormatter GetAtomFeed(string collection);

        [WebGet(UriTemplate = AtomPubServiceBase.AllEntriesTemplate)]
        [OperationContract]
        Atom10FeedFormatter GetAllEntriesAsAtom(string collection);
